...
<div class="bg-white">
    ...
    <div class="relative">
        <div id="banner-carousel" class="relative h-96 overflow-hidden">
            ...
            <div class="banner-slide {{ $index === 0 ? 'active' : '' }} absolute inset-0 transition-opacity duration-700">
                ...
                <div class="absolute inset-0 bg-gradient-to-r from-black via-black/70 to-transparent flex items-center">
                    <div class="text-left text-white pl-8 md:pl-16 max-w-2xl">
                        <h2 class="text-4xl md:text-5xl font-bold mb-4">{{ $banner->title }}</h2>
                        @if($banner->description)
                        <p class="text-lg mb-6">{{ $banner->description }}</p>
                        @endif
                        @if($banner->link)
                        <a href="{{ $banner->link }}" class="px-6 py-3 bg-gradient-to-r from-gold-500 to-gold-600 text-gray-900 rounded-lg font-semibold hover:from-gold-600 hover:to-gold-700 transition-all duration-300 shadow-lg">
                            Shop Now
                        </a>
                        @endif
                    </div>
        ...
        <!-- Banner Navigation -->
        @if($banners->count() > 1)
        <button onclick="prevSlide()" class="absolute top-1/2 left-4 transform -translate-y-1/2 bg-gold-500 bg-opacity-80 hover:bg-opacity-100 rounded-full p-3 shadow-lg transition-all duration-300">
            <i class="fas fa-chevron-left text-gray-900"></i>
        </button>
        <button onclick="nextSlide()" class="absolute top-1/2 right-4 transform -translate-y-1/2 bg-gold-500 bg-opacity-80 hover:bg-opacity-100 rounded-full p-3 shadow-lg transition-all duration-300">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4">
            ...
            <a href="{{ route('category.show', $category->slug) }}" class="text-center group">
                <div class="bg-gray-100 rounded-lg p-6 group-hover:bg-gold-100 transition duration-300">
                    @if($category->image)
                    <img src="{{ asset('storage/' . $category->image) }}" alt="{{ $category->name }}" class="w-16 h-16 mx-auto mb-2 object-cover rounded">
                    @else
    ...
    <!-- Flash Sale -->
    @if($flashSale)
    <div class="bg-gradient-to-r from-gold-50 to-gold-100 py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center mb-8">
                <h2 class="text-3xl font-bold text-gold-700">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div class="grid grid-cols-3 md:grid-cols-6 lg:grid-cols-12 gap-4">
            ...
            <a href="{{ route('brand.show', $brand->slug) }}" class="bg-white p-4 rounded-lg shadow hover:shadow-md transition duration-300 border border-gold-100 hover:border-gold-300 text-center">
                @if($brand->logo)
                <img src="{{ asset('storage/' . $brand->logo) }}" alt="{{ $brand->name }}" class="h-12 mx-auto object-contain">
                @else
