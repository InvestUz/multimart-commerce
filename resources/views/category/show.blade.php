...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="bg-white rounded-lg shadow p-4 mb-6">
        <form action="{{ route('category.show', $category->slug) }}" method="GET" class="flex flex-wrap gap-4">
            ...
            <div>
                <select name="price_range" class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">
            ...
            @if(request()->has('search') || request()->has('sort') || request()->has('price_range') || request()->has('sub_category'))
    ...
    @if($products->count() > 0)
        <div class="mb-4">
            <p class="text-gray-600">
                Showing {{ ($products->currentPage() - 1) * $products->perPage() + 1 }} to {{ min($products->currentPage() * $products->perPage(), $products->total()) }} of {{ $products->total() }} {{ Str::plural('product', $products->total()) }}
            </p>
        </div>
    @endif
                ...
                <div class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
                    <div class="relative">
                        ...
                        @auth
                            <button onclick="toggleWishlist({{ $product->id }})" 
                                ...
                                <i class="fas fa-heart text-2xl {{ auth()->check() && auth()->user()->wishlists()->where('product_id', $product->id)->exists() ? 'text-red-500' : 'text-gray-400' }}"></i>
                            </button>
                        @endauth
                    ...
                    <div class="p-4">
                        ...
                        <a href="{{ route('product.show', $product->slug) }}" class="block">
                            <h3 class="text-lg font-semibold text-gray-900 hover:text-primary mb-2 h-14 overflow-hidden">
                                {{ Str::limit($product->name, 50) }}
                            </h3>
                        </a>

                        <p class="text-sm text-gray-600 mb-2">{{ $product->user->store_name ?? $product->user->name }}</p>
                        ...
                        <div class="mb-4">
                            <div class="flex items-center">
                                ...
                                @if($product->old_price && $product->old_price > $product->price)
                                    <span class="text-sm text-gray-500 line-through ml-2">${{ number_format($product->old_price, 2) }}</span>
                                @endif
                            </div>
                        </div>
                        ...
                        @if($product->stock > 0 && $product->is_active)
                            @auth
                                <form action="{{ route('cart.store') }}" method="POST">
                                    ...
                                    <button type="submit" class="w-full bg-primary hover:bg-green-600 text-white font-semibold py-2 px-4 rounded-lg transition">
                                        Add to Cart
                                    </button>
                                </form>
                            @else
                                <a href="{{ route('login') }}" class="block w-full bg-primary hover:bg-green-600 text-white font-semibold py-2 px-4 rounded-lg text-center transition">
                                    Login to Add
                                </a>
                            @endauth
                        @else
    ...
    @else
        ...
        <div class="bg-white rounded-lg shadow-md p-12 text-center">
            ...
            @if(request()->has('search') || request()->has('sort') || request()->has('price_range') || request()->has('sub_category'))
                <a href="{{ route('category.show', $category->slug) }}" class="inline-block bg-primary hover:bg-green-600 text-white font-semibold py-3 px-6 rounded-lg transition">
                    Clear Filters
                </a>
            @else
                <a href="{{ route('home') }}" class="inline-block bg-primary hover:bg-green-600 text-white font-semibold py-3 px-6 rounded-lg transition">
                    Browse All Products
                </a>
            @endif
        </div>
...
function toggleWishlist(productId) {
    ...
    .then(data => {
        if (data.in_wishlist) {
            ...
            document.querySelector(`[data-wishlist-btn="${productId}"] i`).classList.remove('text-gray-400');
        } else {
